public class Solution {
        ...
        for (int height = minHeight; height <= maxHeight; height++) {
            if (heightToCells.TryGetValue(height, out List<(int x, int y)> cellsForCurrentHeight)) {
                foreach (var cell in cellsForCurrentHeight) {
                    ...
                    DFS(cell, ref hasAccessedAtlantic, ref hasAccessedPacific, heights, hasAccessToBothSeas, hasVisited, dfsVisited);
                    if (hasAccessedAtlantic && hasAccessedPacific) {
                        results.Add(new List<int>{cell.x, cell.y});
                        hasAccessToBothSeas[cell.x][cell.y] = true;
                    }
                    ClearDFSVisited(dfsVisited);
                }
    ...
    public void DFS((int x, int y) cell, ref bool hasAccessedAtlantic, ref bool hasAccessedPacific, int[][] grid, bool[][] hasAccessToBothSeas, bool[][] hasVisited, bool[][] dfsVisited) {
        if (dfsVisited[cell.x][cell.y] && hasAccessToBothSeas[cell.x][cell.y]) {
            hasAccessedAtlantic = true;
            hasAccessedPacific = true;
            // hasVisited[newCell.x][newCell.y] = true;
            return;
        } 
        if (dfsVisited[cell.x][cell.y]) {
            return;
        }
        if (hasVisited[cell.x][cell.y] && hasAccessToBothSeas[cell.x][cell.y]) {
            hasAccessedAtlantic = true;
            hasAccessedPacific = true;
            // hasVisited[newCell.x][newCell.y] = true;
            return;
        ...
        foreach (var direction in directions) {
            ...
            if (CanFlow(cell, newCell, grid)) {
                ...
                DFS(newCell, ref hasAccessedAtlantic, ref hasAccessedPacific, grid, hasAccessToBothSeas, hasVisited, dfsVisited);
            }
        }
    }
    ...
    public void ClearDFSVisited(bool[][] visited ) {
